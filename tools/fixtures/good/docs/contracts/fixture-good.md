...
Aquí cumple el papel de canario del gate: `good/` **sí** declara matriz y
`tools/fixtures/bad/docs/contracts/fixture-bad-orphan.md` **no**. Ese par es lo
que demuestra las dos direcciones (ADR-013).

La fila 3 cubre además la **deuda declarada**: una variante cuya historia todavía
no existe, marcada con `(pendiente)`. Es el estado normal y transitorio entre el
PR del contrato y el de su implementación. **Caduca sola** — en cuanto la
historia exista, dejar el marcador puesto pasa a ser violación, así que
implementar obliga a retirarlo. Sin esa caducidad sería un `'warn'` disfrazado, y
los raíles bloquean, no avisan.
...
| 3 | pulsado | `componentes-fixture-good--pulsado` (pendiente) | light | Deuda DECLARADA: variante aprobada cuya historia aún no existe |
