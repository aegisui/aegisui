...
| [022](ADR-022-cobertura-declarada-y-marcador-que-caduca.md) | El gate `coverage` es asimétrico: la variante declarada y no implementada se DECLARA `(pendiente)` en vez de fallar, y el marcador CADUCA solo (con la historia ya existente, dejarlo puesto es violación) |
| [023](ADR-023-politica-de-dependencias-runtime.md) | Política de dependencias runtime: 8 criterios en vez de "salvo `@angular/*`". `@floating-ui/dom` APROBADA (`dependencies` de `cdk`); `@angular/cdk` RECHAZADA con datos; virtual scroll aplazado a la tabla de datos Pro. Trae sus raíles: `contracts` y `coverage` pasan a cubrir los primitivos headless de `cdk` |
| [024](ADR-024-artefacto-publicable-y-smoke-desde-consumidor-externo.md) | El artefacto publicable se ENSAMBLA (`assemble-dist`: fuente incluida por ADR-001, manifiestos de `cli`/`icons`, `workspace:` resuelto a rango real) y se verifica desde un consumidor EXTERNO con npm (`publish-smoke`, gate required + canario de los 3 fallos). Dentro del monorepo el fallback del CLI hace verde un ADR-003 roto |
